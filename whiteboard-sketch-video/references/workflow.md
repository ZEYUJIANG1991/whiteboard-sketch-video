...
目录：1口播稿 → 2TTS → 3对齐 → 4分镜 → 5生图 → 6project.json → 7渲染 → 8验收 → 9发布文案 → 10封面图
...
用生图模型**直出带文字的封面**（当前模型中文短句渲染可靠，马克笔质感优于
程序贴字；网址等长串字符仍必须程序渲染，勿交给生图）：

- **比例**：3:4 直出（视频号官方推荐，上传首选）。白底的好处：6:7（信息流
  裁切位）和 9:16 用白边扩展导出即可，内容零裁切。
- **安全带**：标题与主角脸放中央竖带——底部会被平台叠加的标题文字遮挡，
  顶部受状态栏/UI 影响。
- **文字结构**：超大标题（1-2 行，视觉主角，钩子或反差）→ 一行副题（说清
  价值）→ 胶囊徽标（如"开源免费"）。**字数越少越不易写错**，每行 ≤8 字为宜。
- **提示词要点**：风格头与正片一致；写明 write these EXACT Chinese characters
  accurately；各行颜色用品牌包三色；主角形象与视频一致；结尾加
  "No other text anywhere. No watermarks."
- **QA**：生成 2 版对比，逐字检查（错字/多字/漏字），有错重生成。
- **交付三尺寸**：3:4（1080×1440）、6:7（1080×1260，白边扩展）、
  9:16（1080×1920，上下白边）。
